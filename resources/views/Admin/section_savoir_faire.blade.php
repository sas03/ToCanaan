@extends('Admin.layout') @section('title', 'Section savoir-faire') @section('content')
<br>

<h3 class="text-uppercase text-jaune">Savoir-faire ({{ $total }})</h3>
...
<div class="row">
  ...
  <div class="col-sm-12 col-lg-4 mt-1">
    <a class="btn bg-jaune" href="{{ route('admin_edit_savoir_faire') }}" role="button">Ajouter un savoir-faire</a>
  </div>
</div>
<br> @if (!empty($savoirFaire))
<div class="row">
  <div class="col-sm-12">
      ...
      <tbody>
        ...
        <tr>
          ...
          <td>
            ...
            @else
              <a href="{{ route('admin_savoir_faire_codes', ['id' => $savoir->id]) }}" class="text-vert"><strong>{{ count($savoir->codes) }}</strong> code(s) ROME</a>
            @endif
          </td>
          <td style="min-width: 190px;">
            {{-- <a type="button" class="btn btn-info" title="Voir la fiche métier" href="" target="_blank"><i class="icon ion-eye"></i></a> --}}
            <a type="button" class="btn btn-warning" title="Modification" href="{{ route('admin_edit_savoir_faire', [$savoir->id]) }}"><i class="icon ion-edit"></i></a>
            <button type="button" class="btn btn-danger delete-savoir-faire" title="Suppression"><i class="icon ion-trash-b"></i></button>
          </td>
        </tr>
        @endforeach
      </tbody>
...
<br>
@if ($savoirFaire instanceof Illuminate\Pagination\LengthAwarePaginator)

  {{ $savoirFaire->links('vendor.pagination.bootstrap-4') }}
...
<script>
  $(function() {
    ...
    $('.delete-savoir-faire').click(function(e) {
      e.preventDefault();

      let tr = $(this).parent().parent();
      let libelle = tr.children().eq(1).text();
      let id = tr.children().eq(0).text();
      let url = "{{ url('admin/savoir-faire/delete') }}" + '/' + id;

      swal({
        title: 'Attention !',
        text: 'Etes-vous sûr de vouloir supprimer le savoir-faire "' + libelle + '" ?',
        type: "warning",
