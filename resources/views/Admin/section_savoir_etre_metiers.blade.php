@extends('Admin.layout') @section('title', 'Section métiers') @section('content')
  ...
  <a href="{{ route('admin_savoir_etre') }}" class="btn-return-back bg-jaune">< Retourner aux savoir-être</a>

<br>

<h3 class="text-uppercase text-vert">Métiers liés au savoir-être <strong>{{ $savoirEtre->nom }}</strong></h3>
...
<div class="row">
  ...
  <div class="col-sm-12 col-lg-12 mb-1">
    ...
    <form class="form-inline col-lg-12" action="{{ route('admin_savoir_etre_metier_add') }}" method="POST">
      {{ csrf_field() }}

      <input type="hidden" name="savoir_etre_id" value="{{ $savoirEtre->id }}">
      <input type="hidden" name="metier_id" value="">

      <div class="input-group col-lg-12">
        <div class="input-group-addon">Lier un nouveau métier à ce savoir-être</div>
  ...
  $(function() {
    ...
    $('.autocomplete').autocomplete({
      ...
      select: function (e, ui) {
        $('input[name=savoir_etre_id]').val(ui.item.id);
      }
    });
