...
<h3 class="text-uppercase text-vert">Métiers liés au battement de coeur <strong>{{ $motivation->nom }}</strong></h3>
...
<div class="row">
  ...
  <div class="col-sm-12 col-lg-12 mb-1">
    ...
    <form class="form-inline col-lg-12" action="{{ route('admin_motivation_metier_add') }}" method="POST">
      {{ csrf_field() }}

      <input type="hidden" name="motivation_id" value="{{ $motivation->id }}">
      <input type="hidden" name="metier_id" value="">

      <div class="input-group col-lg-12">
        <div class="input-group-addon">Lier un nouveau métier à ce battement de coeur</div>
  ...
  $(function() {
    ...
    $('.autocomplete').autocomplete({
      ...
      select: function (e, ui) {
        $('input[name=motivation_id]').val(ui.item.id);
      }
    });
